...
zig build arch-check           # production-only import graph over src/ + examples/ + bench/ + tools/ (AST-based, test-aware): 0 SCCs, 0 band inversions (the ARCHITECTURE.md Layer Stack, encoded as band_table in the tool), every sibling test file forwarded
