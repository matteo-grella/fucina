...
zig build metal-check          # compile-only -Dgpu=metal legs (host fucina/models roots with the Metal shim + frameworks linked, not run): Metal-provider bit-rot gate, macOS hosts only, no device needed
