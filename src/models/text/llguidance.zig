...
const build_options = @import("models_build_options");
...
const ConstraintImpl = struct {
    ...
    /// Compile `grammar` against `tokenizer` (`*const models.text.tokenizer.Tokenizer`
    /// or `*const models.text.spm_tokenizer.Tokenizer`; both are borrowed and must
    ...
    pub fn init(allocator: Allocator, tokenizer: anytype, grammar: Grammar, options: Options) Error!ConstraintImpl {
        const kind: TokKind = switch (@TypeOf(tokenizer)) {
            ...
            *const spm.Tokenizer, *spm.Tokenizer => .{ .spm = tokenizer },
            else => @compileError("Constraint.init: tokenizer must be *const models.text.tokenizer.Tokenizer or *const models.text.spm_tokenizer.Tokenizer, got " ++ @typeName(@TypeOf(tokenizer))),
        };
        const tok_vocab = switch (kind) {
            inline else => |t| t.vocabSize(),
        };
