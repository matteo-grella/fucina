...
pub const State = struct {
    ...
    fn computePrioritiesGemm(self: *State, ctx: *ExecContext, layer_i: usize, q: []const f32) !void {
        ...
        defer qt.deinit();
        var a_t = try ctx.matmul(.f32, .trans_b, &qt, &cat.centroid_t.?);
        defer a_t.deinit();
        var p_t: ?Tensor = null;
        defer if (p_t) |*t| t.deinit();
        if (rank > 0) p_t = try ctx.matmul(.f32, .trans_b, &qt, &cat.eig_t.?);
        const qq = self.qq[0 .. self.q_heads * d];
        for (qq, q[0 .. self.q_heads * d]) |*o, x| o.* = x * x;
        var qq_t = try ctx.fromSlice(.f32, .{ self.q_heads, d }, qq);
        defer qq_t.deinit();
        var dv_t = try ctx.matmul(.f32, .trans_b, &qq_t, &cat.diag_t.?);
        defer dv_t.deinit();
