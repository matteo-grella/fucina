...
pub const Model = struct {
    /// Decoder-contract decode state (`models.decoder`): a full `Session`
    ...
    /// Decoder-contract entry (`models.decoder`): process `tokens` at
...
const hostMatrix = weights.hostMatrix;
...
fn hcSplitSinkhorn(mix: []const f32, scale: []const f32, base: []const f32, n_hc: usize, iters: usize) HcSplit {
    ...
    for (0..n_hc) |i| out.pre[i] = fucina.ops.sigmoidStableScalar(mix[i] * scale[0] + base[i]) + eps;
    for (0..n_hc) |i| out.post[i] = 2.0 * fucina.ops.sigmoidStableScalar(mix[n_hc + i] * scale[1] + base[n_hc + i]);
...
fn outputLogitsWithInto(self: *Model, ctx: *ExecContext, streams: []const f32, head_hc: *const HcModule, out_norm: []const f32, dst: []f32) ![]f32 {
    ...
    for (0..cfg.n_hc) |i| merge_w[i] = fucina.ops.sigmoidStableScalar(pre_vals[i] * head_hc.scale[0] + head_hc.base[i]) + cfg.hc_eps;
