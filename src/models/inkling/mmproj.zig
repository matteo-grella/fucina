...
pub const MmProj = struct {
    ...
    pub fn loadGgufFromFile(ctx: *ExecContext, file: *gguf.File) !MmProj {
        ...
        return .{
            ...
            .mm_profile = fucina.parallel.env.flag("FUCINA_MM_PROFILE"),
            .dmel_embd = dmel_embd,
    ...
    fn packedLinear(ctx: *ExecContext, dense_rhs: *const fucina.PackedRhs(.f32), x: []const f32, rows: usize, in_dim: usize) ![]f32 {
        ...
        defer output.deinit();
        try ctx.matmulQuantInto(&output, .{ .plain = &input }, dense_rhs, .{});
        return out;
    }
...
// deliberate variant of models/profile.zig's profile-pointer-gated pair.
