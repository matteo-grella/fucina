...
/// for that call. Overrides must leave KV state untouched; `models.research.subq`
...
pub const Model = struct {
    /// Decoder-contract decode state (`models.decoder`): the shared KV cache.
    ...
    pub fn loadGgufOptions(ctx: *ExecContext, io: std.Io, path: []const u8, config: Descriptor, options: LoadOptions) !Model {
        // mmap, matching the CLI (apps/qwen3/main.zig) and the other loaders:
    ...
    if (ffn_in.dim(.seq) >= 12 and dense.input_proj == .fused) {
        ...
        switch (dense.down_proj) {
            .packed_quant => |*pq| switch (pq.*) {
                ...
                inline .q5_k, .q6_k, .q8_0 => |*down| return denseFfnFusedDown(ctx, io, gate_up_weight, &down.packed_rhs, ffn_in, profile),
            },
            else => {},
        }
    }
    ...
    if (input.dim(.seq) >= 12) {
        switch (dense.down_proj) {
            .packed_quant => |*pq| switch (pq.*) {
                ...
                inline .q5_k, .q6_k, .q8_0 => |*down| return denseFfnFusedDownNormed(ctx, io, gate_up_weight, &down.packed_rhs, input, norm_weight, eps, profile),
            },
            else => {},
        }
    }
