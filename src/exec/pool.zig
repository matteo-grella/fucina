...
//!
//! Domain module: every op receives an explicit `*ExecContext`. Validates
...
pub fn pool2d(ctx: *ExecContext, comptime kind: PoolKind, input: *const Tensor, kernel: [2]usize, stride: [2]usize, pad: [2]usize) !Tensor {
...
/// border reads as −inf).
pub fn maxPool2d(ctx: *ExecContext, input: *const Tensor, kernel: [2]usize, stride: [2]usize, pad: [2]usize) !Tensor {
    return pool2d(ctx, .max, input, kernel, stride, pad);
}

/// 2-D average pool, channel-last rank-3; averages the valid taps only
/// (ONNX `count_include_pad=0`).
pub fn avgPool2d(ctx: *ExecContext, input: *const Tensor, kernel: [2]usize, stride: [2]usize, pad: [2]usize) !Tensor {
    return pool2d(ctx, .avg, input, kernel, stride, pad);
}

/// VJP of the avg pool: scatter `gy/valid_count` over each window. `gy` is
/// `[OH,OW,C]`; result is `[in_h,in_w,C]`.
pub fn avgPool2dBackward(ctx: *ExecContext, gy: *const Tensor, in_h: usize, in_w: usize, kernel: [2]usize, stride: [2]usize, pad: [2]usize) !Tensor {
...
pub fn maxPool2dBackward(ctx: *ExecContext, input: *const Tensor, gy: *const Tensor, kernel: [2]usize, stride: [2]usize, pad: [2]usize) !Tensor {
...
pub fn upsample2xNearest(ctx: *ExecContext, input: *const Tensor) !Tensor {
    const view = try input.rankView(3);
...
pub fn upsample2xNearestBackward(ctx: *ExecContext, gy: *const Tensor) !Tensor {
    const view = try gy.rankView(3);
    if (view.shape[0] % 2 != 0 or view.shape[1] % 2 != 0) return tensor.TensorError.ShapeMismatch;
    return pool2d(ctx, .sum, gy, .{ 2, 2 }, .{ 2, 2 }, .{ 0, 0 });
}
