...
const ParallelConfig = @import("common.zig").ParallelConfig;

/// The old per-name entry shape over the one `gemm2D` request, so the
/// pooled-vs-serial specs below stay tabular.
fn gemm2DEntry(comptime g: backend_ops.QuantGemm) fn (ParallelConfig, []f32, backend_ops.LhsOf(g), backend_ops.RhsOf(g), usize, usize, usize) void {
    return struct {
        fn run(pc: ParallelConfig, out: []f32, lhs: backend_ops.LhsOf(g), rhs: backend_ops.RhsOf(g), m: usize, n: usize, k: usize) void {
            matmul_quant.gemm2D(pc, g, out, lhs, rhs, m, n, k);
        }
    }.run;
}
...
fn expectPooledTQ2_0F32MatchesSerial(m: usize, n: usize, k: usize, seed: u64) !void {
    ...
    defer pool.deinit();
    matmul_quant.gemm2D(.{ .pool = &pool }, .{ .weight = .tq2_0, .lhs = .f32 }, pooled, lhs, &rhs, m, n, k);
...
fn expectPooledTQ2_0Int8MatchesSerial(m: usize, n: usize, k: usize, seed: u64) !void {
    ...
    try pool.init(.{ .allocator = allocator, .max_workers = 3 });
    defer pool.deinit();
    matmul_quant.gemm2D(.{ .pool = &pool }, .{ .weight = .tq2_0, .lhs = .q8_k }, pooled, qlhs, &rhs, m, n, k);
...
test "pooled dispatch matches serial: plain-block formats" {
    ...
    inline for (.{ .{ row_m, row_n, row_k }, .{ col_m, col_n, col_k } }) |shape| {
        ...
        inline for (.{
            .{ .q4_k, qm.types.QuantizedMatmulRhsQ4_K, dtype_mod.BlockQ4_K, gemm2DEntry(.{ .weight = .q4_k, .lhs = .q8_k }), qm.q4_k.matmulQ4_KRhsRange },
            .{ .q5_k, qm.types.QuantizedMatmulRhsQ5_K, dtype_mod.BlockQ5_K, gemm2DEntry(.{ .weight = .q5_k, .lhs = .q8_k }), qm.q5_k.matmulQ5_KRhsRange },
            .{ .q6_k, qm.types.QuantizedMatmulRhsQ6_K, dtype_mod.BlockQ6_K, gemm2DEntry(.{ .weight = .q6_k, .lhs = .q8_k }), qm.q6_k.matmulQ6_KRhsRange },
        }) |spec| {
        ...
        // trit/scale patterns are arbitrary).
        {
            ...
            const rhs: qm.types.QuantizedMatmulRhsQ2_K = .{ .allocator = null, .blocks = rblocks, .k = k, .n = n, .blocks_per_column = k / qk_k };
            try expectPooledMatchesSerial(gemm2DEntry(.{ .weight = .q2_k, .lhs = .q8_k }), qm.cold.matmulQ2_KRhsRange, lhs_q8k, &rhs, m, n, k);
        }
        {
            const rblocks = try randomBlocks(dtype_mod.BlockQ3_K, allocator, &prng, n * (k / qk_k));
            defer allocator.free(rblocks);
            for (rblocks) |*bl| bl.d = f16Bits(0.01);
            const rhs: qm.types.QuantizedMatmulRhsQ3_K = .{ .allocator = null, .blocks = rblocks, .k = k, .n = n, .blocks_per_column = k / qk_k };
            try expectPooledMatchesSerial(gemm2DEntry(.{ .weight = .q3_k, .lhs = .q8_k }), qm.cold.matmulQ3_KRhsRange, lhs_q8k, &rhs, m, n, k);
        }
        ...
        // Q8_0/Q4_0 against the Q8_0 activations (rows containers).
        {
            ...
            const rhs: qm.types.QuantizedMatmulRhsQ8_0 = .{ .allocator = null, .blocks = rblocks, .blocks_per_column = k / 32, .k = k, .n = n };
        ...
        }
        {
            ...
            const rhs: qm.types.QuantizedMatmulRhsQ4_0 = .{ .allocator = null, .blocks = q4_blocks, .blocks_per_column = k / 32, .k = k, .n = n };
...
test "pooled dispatch matches serial: interleaved-pack formats" {
    ...
    inline for (.{
        ...
        .{ .q8_0, dtype_mod.BlockQ8_0, false, qm.q8_0.packMatmulRhsQ8_0x4, gemm2DEntry(.{ .weight = .q8_0, .rhs = .x4, .lhs = .q8_0 }), qm.q8_0.matmulQ8_0x4RhsRange, .{ .{ row_m, row_n, row_k }, .{ col_m, col_n, col_k } } },
        .{ .q4_k, dtype_mod.BlockQ4_K, true, qm.q4_k.packMatmulRhsQ4_Kx4, gemm2DEntry(.{ .weight = .q4_k, .rhs = .x4, .lhs = .q8_k }), qm.q4_k.matmulQ4_Kx4RhsRange, .{ .{ row_m, row_n, row_k }, .{ col_m, col_n, col_k } } },
        .{ .q4_k, dtype_mod.BlockQ4_K, true, qm.q4_k.packMatmulRhsQ4_Kx8, gemm2DEntry(.{ .weight = .q4_k, .rhs = .x8, .lhs = .q8_k }), qm.q4_k.matmulQ4_Kx8RhsRange, .{ .{ row_m, row_n, row_k }, .{ col_m, col_n, col_k }, .{ 128, 128, qk_k } } },
        .{ .q4_k, dtype_mod.BlockQ4_K, true, qm.q4_k.packMatmulRhsQ4_Kx2Mmla, gemm2DEntry(.{ .weight = .q4_k, .rhs = .x2mmla, .lhs = .q8_k }), qm.q4_k.matmulQ4_Kx2MmlaRhsRange, .{ .{ row_m, row_n, row_k }, .{ col_m, col_n, col_k }, .{ 128, 128, qk_k } } },
        .{ .q5_k, dtype_mod.BlockQ5_K, true, qm.q5_k.packMatmulRhsQ5_Kx8, gemm2DEntry(.{ .weight = .q5_k, .rhs = .x8, .lhs = .q8_k }), qm.q5_k.matmulQ5_Kx8RhsRange, .{ .{ row_m, row_n, row_k }, .{ col_m, col_n, col_k }, .{ 128, 128, qk_k } } },
        .{ .q6_k, dtype_mod.BlockQ6_K, true, qm.q6_k.packMatmulRhsQ6_Kx4, gemm2DEntry(.{ .weight = .q6_k, .rhs = .x4, .lhs = .q8_k }), qm.q6_k.matmulQ6_Kx4RhsRange, .{ .{ row_m, row_n, row_k }, .{ col_m, col_n, col_k } } },
    }) |spec| {
...
test "pooled dispatch matches serial: lane-packed LHS entries" {
    ...
    inline for (.{
        ...
        .{ .q4_k, dtype_mod.BlockQ4_K, qm.types.BlockQ8_Kx4, qm.q8k.quantizeRowsQ8_Kx4PaddedInto, qm.q4_k.packMatmulRhsQ4_Kx8, gemm2DEntry(.{ .weight = .q4_k, .rhs = .x8, .lhs = .q8_kx4 }), qm.q4_k.matmulQ4_Kx8Q8_Kx4RhsRange, .{ .{ 64, 64, qk_k }, .{ 4, col_n, col_k }, .{ row_m, row_n, row_k } } },
        .{ .q5_k, dtype_mod.BlockQ5_K, qm.types.BlockQ8_Kx4, qm.q8k.quantizeRowsQ8_Kx4PaddedInto, qm.q5_k.packMatmulRhsQ5_Kx8, gemm2DEntry(.{ .weight = .q5_k, .rhs = .x8, .lhs = .q8_kx4 }), qm.q5_k.matmulQ5_Kx8Q8_Kx4RhsRange, .{ .{ 64, 64, qk_k }, .{ 4, col_n, col_k }, .{ row_m, row_n, row_k } } },
        .{ .q4_k, dtype_mod.BlockQ4_K, qm.types.BlockQ8_Kx2Mmla, qm.q8k.quantizeRowsQ8_Kx2MmlaInto, qm.q4_k.packMatmulRhsQ4_Kx2Mmla, gemm2DEntry(.{ .weight = .q4_k, .rhs = .x2mmla, .lhs = .q8_kx2mmla }), qm.q4_k.matmulQ4_Kx2MmlaQ8_Kx2MmlaRhsRange, .{ .{ 64, 64, qk_k }, .{ 2, col_n, col_k }, .{ 128, 128, qk_k } } },
        .{ .q8_0, dtype_mod.BlockQ8_0, qm.types.BlockQ8_0x4, qm.q8_0.quantizeRowsQ8_0x4PaddedInto, qm.q8_0.packMatmulRhsQ8_0x4, gemm2DEntry(.{ .weight = .q8_0, .rhs = .x4, .lhs = .q8_0x4 }), qm.q8_0.matmulQ8_0x4PackedRhsRange, .{ .{ row_m, row_n, row_k }, .{ 4, col_n, col_k }, .{ 128, col_n, col_k } } },
    }) |spec| {
        inline for (spec[7]) |shape| {
            ...
            var a = try Tensor.fromSlice(allocator, &.{ m, k }, acts);
            defer a.deinit();
            const group: usize = if (spec[2] == qm.types.BlockQ8_Kx2Mmla) 2 else 4;
