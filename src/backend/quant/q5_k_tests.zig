...
test "ggml_q5_k x8 packed matmul matches plain q5_k matmul" {
    ...
    var rhs_plain = try q8k.quantizedMatmulRhsQ5_KFromBlocks(allocator, dtype_mod.qk_k_block_size, 8, &rhs_blocks);
    defer rhs_plain.deinit();
    var rhs_packed = try q5_k.packMatmulRhsQ5_Kx8(allocator, &rhs_blocks, 8, dtype_mod.qk_k_block_size, 1);
    defer rhs_packed.deinit();

    var expected: [32]f32 = undefined;
    var actual: [32]f32 = undefined;
    q5_k.matmulQ5_KRhsRange(&expected, &lhs_blocks, &rhs_plain, 4, 8, 0, 4);
    q5_k.matmulQ5_Kx8RhsRange(&actual, &lhs_blocks, &rhs_packed, 4, 8, 0, 4);

    for (expected, actual) |ref, got| {
        try std.testing.expectApproxEqAbs(ref, got, 1e-3);
    }

    const lhs_x4 = try q8k.packRowsQ8_Kx4(allocator, &lhs_blocks, 4, dtype_mod.qk_k_block_size, 1);
    defer allocator.free(lhs_x4);
    var actual_x4: [32]f32 = undefined;
    q5_k.matmulQ5_Kx8Q8_Kx4RhsRange(&actual_x4, lhs_x4, &rhs_packed, 4, 8, 0, 4);
...
test "Q5_K compact-vs-packed cross-layout matmul is bit-identical at decode shapes (m=1,2,3)" {
    ...
    var rhs_plain = try q8k.quantizedMatmulRhsQ5_KFromBlocks(allocator, k, n, blocks);
    defer rhs_plain.deinit();
    var rhs_packed = try q5_k.packMatmulRhsQ5_Kx8(allocator, blocks, n, k, bpc);
    defer rhs_packed.deinit();
    ...
    inline for ([_]usize{ 1, 2, 3 }) |m| {
        ...
        q5_k.matmulQ5_KRhsRange(out_compact, qlhs, &rhs_plain, m, n, 0, m);
        q5_k.matmulQ5_Kx8RhsRange(out_packed, qlhs, &rhs_packed, m, n, 0, m);
...
test "Q5_K column-outer matmul matches row-outer tile" {
    ...
    var rhs = try q8k.quantizedMatmulRhsQ5_KFromBlocks(allocator, k, n, blocks);
    defer rhs.deinit();
    ...
    q5_k.matmulQ5_KRhsTile(out_tile, qlhs, &rhs, n, 0, m, 0, n);
    q5_k.matmulQ5_KRhsCompactColOuter(out_col, qlhs, &rhs, n, 0, m, 0, n);
...
test "Q5_K lane-packed Q8_Kx4 col-outer is bit-identical on the same activations" {
    ...
    const qlhs = try qm.q8k.quantizeRowsQ8_K(allocator, &dense);
    defer allocator.free(qlhs);
    const qlhs_x4 = try q8k.packRowsQ8_Kx4(allocator, qlhs, m, k, bpc);
    ...
    q5_k.matmulQ5_KRhsCompactColOuter(out_col, qlhs, &rhs, n, 0, m, 0, n);
    q5_k.matmulQ5_KCompactQ8_Kx4ColOuter(out_x4, qlhs_x4, &rhs, n, m, 0, n);
...
test "Q5_K lane-packed Q8_Kx4 col-outer matches row-outer tile with padded tail" {
    ...
    defer allocator.free(qlhs_x4);
    try qm.q8k.quantizeRowsQ8_Kx4PaddedInto(qlhs_x4, &dense);
...
test "Q5_K col-outer kernels: split column ranges are bit-identical to full range" {
    ...
    for ([_]usize{ 1, 3 }) |mt| {
        q5_k.matmulQ5_KRhsTile(out_full, qlhs, &rhs, n, 0, mt, 0, n);
        q5_k.matmulQ5_KRhsTile(out_split, qlhs, &rhs, n, 0, mt, 0, split);
        q5_k.matmulQ5_KRhsTile(out_split, qlhs, &rhs, n, 0, mt, split, n);
        try std.testing.expectEqualSlices(f32, out_full[0 .. mt * n], out_split[0 .. mt * n]);
    }

    // Per-row column-outer: the m >= 4 arm.
    q5_k.matmulQ5_KRhsCompactColOuter(out_full, qlhs, &rhs, n, 0, m, 0, n);
    q5_k.matmulQ5_KRhsCompactColOuter(out_split, qlhs, &rhs, n, 0, m, 0, split);
    q5_k.matmulQ5_KRhsCompactColOuter(out_split, qlhs, &rhs, n, 0, m, split, n);
    ...
    defer allocator.free(qlhs_x4);
    qm.q8k.packRowsQ8_Kx4PaddedInto(qlhs_x4, qlhs, m, bpc);
    q5_k.matmulQ5_KCompactQ8_Kx4ColOuter(out_full, qlhs_x4, &rhs, n, m, 0, n);
    q5_k.matmulQ5_KCompactQ8_Kx4ColOuter(out_split, qlhs_x4, &rhs, n, m, 0, split);
    q5_k.matmulQ5_KCompactQ8_Kx4ColOuter(out_split, qlhs_x4, &rhs, n, m, split, n);
    try std.testing.expectEqualSlices(f32, out_full, out_split);
}
...
fn fillRandomBlockQ5_Kx8(block: *qm.types.BlockQ5_Kx8, random: std.Random) void {
...
fn randomAcc2(random: std.Random) [2]common.QKV4f32 {
    var acc: [2]common.QKV4f32 = undefined;
    for (&acc) |*half| {
...
fn expectAcc2BitEqual(expected: [2]common.QKV4f32, got: [2]common.QKV4f32) !void {
    inline for (0..2) |h| {
...
fn expectAcc4x2BitEqual(expected: [4][2]common.QKV4f32, got: [4][2]common.QKV4f32) !void {
...
fn checkPlainArms(lhs: *const dtype_mod.BlockQ8_K, rhs: *const qm.types.BlockQ5_Kx8, acc0: [2]common.QKV4f32) !void {
    var ref = acc0;
...
fn checkPackedArms(lhs: *const types.BlockQ8_Kx4, rhs: *const qm.types.BlockQ5_Kx8, acc0: [4][2]common.QKV4f32) !void {
    var ref = acc0;
...
fn fillEdgeBlockQ5_Kx8(block: *qm.types.BlockQ5_Kx8, qs_value: u8, scale_value: u8) void {
...
test "ggml_q5_kx8 plain-lhs SIMD arms match the scalar arm bit-exactly" {
    ...
    while (iter < 200) : (iter += 1) {
        ...
        var rhs: qm.types.BlockQ5_Kx8 = undefined;
...
test "ggml_q5_kx8 packed Q8_Kx4 SIMD arms match the scalar arm bit-exactly" {
    ...
    const acc_zero: [4][2]common.QKV4f32 = .{
        .{ @splat(0), @splat(0) },
        .{ @splat(0), @splat(0) },
        .{ @splat(0), @splat(0) },
        .{ @splat(0), @splat(0) },
...
test "ggml_q5_kx8 matmul entry points match the scalar-arm reference bit-exactly" {
    ...
    q5_k.matmulQ5_Kx8RhsRange(got, qlhs, &rhs_packed, m, n, 0, m);

    for (0..m) |i| {
        var j: usize = 0;
        while (j < n) : (j += 8) {
            const rhs_group = rhs_packed.groupBlocks(j / 8);
            var acc: [2]common.QKV4f32 = .{ @splat(0), @splat(0) };
            for (0..bpc) |bi| {
    ...
    const m4 = 8;
    const qlhs_x4 = try q8k.packRowsQ8_Kx4(allocator, qlhs[0 .. m4 * bpc], m4, k, bpc);
    defer allocator.free(qlhs_x4);
    const got_x4 = try allocator.alloc(f32, m4 * n);
    defer allocator.free(got_x4);
    q5_k.matmulQ5_Kx8Q8_Kx4RhsRange(got_x4, qlhs_x4, &rhs_packed, m4, n, 0, m4);
...
const dot_tiers = [_]isa.Tier{ .x86_vnni, .x86_avx2, .portable };
...
test "q5_k 4-row lane dot SIMD arms match the scalar reference" {
    ...
    while (iter < 100) : (iter += 1) {
        ...
        inline for (0..8) |subblock| {
            const ref: [4]i32 = common.dot4RowsSubblockQ8_Kx4Scalar(&a, subblock, wv);
            inline for (dot_tiers) |tier| {
                try std.testing.expectEqual(ref, @as([4]i32, common.dot4RowsSubblockQ8_Kx4Simd(tier, &a, subblock, wv)));
            }
        }
    }
...
test "q5_k row-outer tile matches the scalar row dot bit-exactly" {
    ...
    const got = try allocator.alloc(f32, m * n);
    defer allocator.free(got);
    q5_k.matmulQ5_KRhsTile(got, qlhs, &rhs, n, 0, m, 0, n);
