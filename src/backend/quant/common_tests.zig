...
test "common.dotU8I8x16Portable matches the scalar reference (nibble + stress domains)" {
    ...
    while (iter < 200) : (iter += 1) {
        ...
        try std.testing.expectEqual(refDotU8I8(&aa, &bb), common.dotU8I8x16Portable(aa, bb));
    ...
    try std.testing.expectEqual(@as(i32, 16 * 128 * 127), common.dotU8I8x16Portable(a_edge, b_hi));
    try std.testing.expectEqual(@as(i32, 16 * 128 * -127), common.dotU8I8x16Portable(a_edge, b_lo));
    // -128 on the signed side with nibble weights (the Q4_K worst case).
    const a_nib: [16]u8 = @splat(15);
    const b_min: [16]i8 = @splat(-128);
    try std.testing.expectEqual(@as(i32, 16 * 15 * -128), common.dotU8I8x16Portable(a_nib, b_min));
}

test "common.dotI8x16Portable sign-trick domain: extremes and saturation stress" {
    ...
    while (iter < 200) : (iter += 1) {
        ...
        try std.testing.expectEqual(refDotI8I8(&aa, &bb), common.dotI8x16Portable(aa, bb));
    }
    ...
    const b_hi: [16]i8 = @splat(127);
    const b_lo: [16]i8 = @splat(-127);
    try std.testing.expectEqual(@as(i32, 16 * -128 * 127), common.dotI8x16Portable(a_min, b_hi));
    try std.testing.expectEqual(@as(i32, 16 * -128 * -127), common.dotI8x16Portable(a_min, b_lo));
    ...
    try std.testing.expectEqual(refDotI8I8(&am, &bm), common.dotI8x16Portable(am, bm));
}
