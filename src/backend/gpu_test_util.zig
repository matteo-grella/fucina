...
//!
//! Provider ABI integers are private (see `gpu_provider.zig`), so the format-keyed
...
/// The `DType` a `QuantFormat` tag names. Tags are spelled
...
pub fn dtypeForFormat(comptime fmt: anytype) DType {
    ...
    if (!@hasField(DType, name)) @compileError("no DType named `" ++ name ++ "` for format tag");
    return @field(DType, name);
}

/// The block struct backing a format tag (`dtype.block_formats`).
