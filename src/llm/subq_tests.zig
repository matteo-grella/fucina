...
test "subq q8_0 packed format matches a dequantized-oracle at tau 0" {
    ...
    for (0..q_heads) |head_i| {
        ...
        try @import("fucina").internal.backend_mod.quantized_matmul.quantizeRowQ8_0Into(q_q8[0..bpr], query);
        var qdeq: [d]f32 = undefined;
        try @import("fucina").internal.backend_mod.quantized_matmul.dequantizeRowQ8_0Into(&qdeq, q_q8[0..bpr]);
        var z: f64 = 0;
        var want: [d]f64 = @splat(0);
        ...
        for (0..n_packed) |row| {
            try @import("fucina").internal.backend_mod.quantized_matmul.dequantizeRowQ8_0Into(&krow, plan.packed_k_q8[row * bpr ..][0..bpr]);
            try @import("fucina").internal.backend_mod.quantized_matmul.dequantizeRowQ8_0Into(&vrow, plan.packed_v_q8[row * bpr ..][0..bpr]);
            var dot: f64 = 0;
