...
//! (goldens-qwen3tts/dump-greedy: CustomVoice Aiden, "The quick brown fox
...
const dump_dir = "goldens-qwen3tts/dump-greedy";
...
test "qwen3tts talker: seeded sampling replays the oracle draw-for-draw" {
    ...
    defer file.deinit();
    const bytes = readFile(allocator, "goldens-qwen3tts/dump-seed42/codes-full.bin") catch return error.SkipZigTest;
    allocator.free(bytes);
    var codes_dump = blk: {
        const b = try readFile(allocator, "goldens-qwen3tts/dump-seed42/codes-full.bin");
