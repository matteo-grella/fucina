...
const rvq_path = "testdata/qwen3tts/codes-137.rvq";
...
test "qwen3tts codec: stage bisect vs oracle dumps (debug, needs dump-codec/)" {
    ...
    const tfm_dump = try loadDump(allocator, "testdata/qwen3tts/dump-codec/codec-tfm.bin");
    defer allocator.free(tfm_dump.data);
    const up_dump = try loadDump(allocator, "testdata/qwen3tts/dump-codec/codec-up.bin");
