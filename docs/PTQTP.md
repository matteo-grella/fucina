...
> run resident → run streamed) is the Recipe section below; this section
> is the tool reference.
...
on-disk format, and the full quality ladder are the rest of this
document.
...
— deliberate, and documented as graceful degradation under Configuration guidance above.
...
- `--ptqtp=K` — trit planes per matrix. K=1 ≈ 2.06 bpw (maximum
  ...
  ≈ 0.17), K=3 ≈ 6.2 bpw (near-parity; rel_err ≈ 0.067). §Measured
  below has the NLL ladder.
- `--ptqtp-tie` — scale-tied fit (K ≥ 2): plane scales locked to the
  ...
  §Scale-tied below has the ladder). At K=2 the stamped file
...
- **K per use case**: K=1 for maximum-compression experiments, K=2 as
  ...
  the source at ~1.9× decode speed (see `TERNARY.md` for
