...
`src/lora_tests.zig` / `src/models/qwen3/train_tests.zig` /
...
- **Scopes.** One scope per micro-batch is recommended: each graph is freed
  ...
  legal (`src/models/qwen3/train_tests.zig` "rope cache: gradient accumulation…"
...
- **Checkpoints only at window boundaries.** Accumulated gradients live only
  ...
  (the optional `accum_steps` field in `fucina_models.train.trainer_state.TrainerState`
...
**Qwen3 fine-tuning** (`models.qwen3.train`, `src/models/qwen3/train.zig`).
...
**Data comes from `models.text.data` (`src/models/text/data.zig`).** The SFT
...
- **Full-stack finite differences** (`src/models/qwen3/train_tests.zig`).
...
- **PyTorch golden parity** (`src/models/qwen3/train_golden_tests.zig`,
...
- **Serving-format parity gate** (`src/models/qwen3/train_tests.zig`): at
...
`fucina_models.train.trainer_state.TrainerState` — there
