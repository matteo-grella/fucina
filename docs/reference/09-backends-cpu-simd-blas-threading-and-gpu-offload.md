...
| `vector/rows.zig` | fused row kernels (softmax/logsumexp rows + strided inner-lane arms, layer/RMS-norm rows and backward stats, cross-entropy/distillation rows, dropout, scatter-add, gated activations, the fused activation+quantize workers) with their Task payloads (`backend.rows`) |
...
(`ExecContext.forRange` with the `innerLaneParts` cap): lanes are independent and scratch
...
- **The BLIS-style blocked packed GEMM** (`gemm_blocked.zig`) — the
  ...
  `kc_max = 512`, `mc_max = 256`, `nc_max = 1024`. On aarch64 (the
  ic-block-only split) the row block follows the row count and the team
  (`blockRowsFor`): when `ceil(m / mc)` blocks would not cover the
  participants, `mc` shrinks to the smallest `mr`-aligned block that
...
- Block-quantized weights: the container types `QuantizedMatmulRhsQ8_0x4`,
  ...
  `weights.packRhs(ctx)` / `packRhsAs(ctx, fucina.quant.types.QuantizedMatmulRhsQ4_Kx8)`
