...
- **The f32 arm** — a free list of `*storage.Buffer`. `ctx.empty(.f32, ...)`
  acquires from it (`src/exec/runtime.zig:441-447`). In an LLM forward
...
- **The byte-slab arm** — a free list of 64-byte-aligned, 4096-byte-rounded raw
  ...
  non-f32 dtype through `BufferPool.acquireTyped` (`src/exec/runtime.zig:444`), which
  ...
  `matmulHalfRhs` (`src/exec/matmul.zig`), typed gathers, typed
...
slot (`src/models/text/kv_cache.zig:286-298`).
...
- **Training variables retain their inputs.** Backward records save operand
  ...
  `cloneView` bumps the refcount (`src/tensor.zig:208-212`).
...
- **`KvCache.reset()` does not zero or free** — it only sets `len = 0`
  (`src/models/text/kv_cache.zig:217-219`); stale f16 data is simply overwritten on the
