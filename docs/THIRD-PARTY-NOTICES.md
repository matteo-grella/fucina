...
- **Qwen2/GPT-2 pretokenizer** (`src/models/text/tokenizer.zig`): faithful port of
...
- **SentencePiece tokenizer** (`src/models/text/spm_tokenizer.zig`): faithful port of
  `llm_tokenizer_spm` (`src/llama-vocab.cpp`); byte-exact vs `llama-tokenize`.
- **Unicode classification tables** (`src/models/text/unicode_categories.zig`,
...
- **Model graphs** (`src/models/gemma/model.zig`, `src/models/qwen35/model.zig`):
  implemented with llama.cpp's `src/models/*.cpp` as source of truth,
  validated by logit parity. **DiffusionGemma**
  (`src/models/diffusion_gemma/model.zig`) was implemented from llama.cpp draft
  PR #24423 (at the time unmerged) plus the Hugging Face reference
  implementation. **Inkling** (`src/models/inkling/`, an architecture by
  ...
  classification tables (`src/models/text/unicode_categories.zig`) is still generated
...
- **DeepSeek V4 Flash reference numerics** (`src/models/deepseek4/model.zig`):
...
- **joyai-llm pre-tokenizer** (`src/models/text/tokenizer.zig`): faithful port of
...
- **Pocket TTS v2** (`src/models/pockettts/`, `examples/pockettts/`): a pure-Zig
...
- **Qwen3-TTS** (`src/models/qwen3tts/`, `examples/qwen3tts/`): the talker, the
