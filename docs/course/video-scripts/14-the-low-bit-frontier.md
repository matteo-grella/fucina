...
the sign.
**Visual:** Code shot: `BlockTQ2_0` at `src/dtype.zig:225-228` with the
size assert at `src/dtype.zig:265` (`== 66`) beneath it; then the chapter's
compile-checked identity test at
`docs/course/14-the-low-bit-frontier.md:58-73`; end on the production inner
loop at `src/backend/quant/ternary.zig:360-371` with
...
## Asset list
- **Repo code shots**:
  - `src/dtype.zig:225-228` — `BlockTQ2_0` struct; pair with the
    `@sizeOf == 66` assert at `src/dtype.zig:265`.
  - `src/backend/quant/ternary.zig:360-371` — the tile inner loop
