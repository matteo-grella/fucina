...
| ag + training/serialization | src/ag.zig, src/ag/**, src/optim.zig, src/optim/**, src/es.zig, src/es/**, src/gguf.zig, src/gguf/**, src/lora.zig, … |
| tagged      | src/tag_ops.zig (tag-ops library) |
| exec        | src/exec.zig, src/exec/** (eager runtime) |
| store       | src/store/** (disk-streamed block stores: the out-of-core MoE expert tier) |
| backend     | src/backend.zig, src/backend/** (the one CPU kernel provider) |
| tags        | src/tags.zig (comptime tag algebra) |
| tensor      | src/tensor.zig (raw tensor) |
| primitives  | src/thread.zig, src/parallel.zig, src/tuning.zig |
| core        | src/dtype.zig, src/storage.zig, src/accelerator.zig, src/rng.zig, and the std-only leaves |
```

One rule: **a band may depend only on bands at or below it.** Read the consequences off the table. The tensor does not know autograd exists. The backend does not know models exist. The exec runtime never contains family-specific logic — when Gemma's MoE and Qwen's MoE both needed batched expert scheduling, the shared scheduler landed *once* in `src/moe/chain.zig` (the moe band) and both families import it downward, rather than exec growing an `if (model == .gemma)` upward. And the `fucina_models` module imports the `fucina` *module* — the public surface plus the explicit `fucina.internal` escape hatch — never individual `src/*.zig` files (`docs/ARCHITECTURE.md`, *Layering And Enforcement*).
...
The layering has teeth even at the API boundary. Fucina's raw f32 tensor — the workhorse of Chapters 3–6 — is deliberately *not* part of the public surface, and the rule is enforced by the compiler itself, in `src/fucina.zig:95-110`:
...
> **Zig note** — The top bands are also *module* boundaries, not just directories. `build.zig` wires three library modules — `fucina` rooted at `src/fucina.zig`, `fucina_models` rooted at `src/models.zig`, and `fucina_serving` rooted at `src/serving.zig` — and the models module's only library import is the `fucina` module root (`docs/ARCHITECTURE.md`, *Build And Verification*). Two Zig rules give the boundary teeth: named module imports like `@import("fucina")` resolve only if the build declaration hands them over, and every file must belong to exactly one module. So "models files never import individual `src/*.zig` files" is not a convention — an `@import("../exec.zig")` from inside `src/models/` is a compile error ("file exists in modules 'fucina' and 'fucina_models'"), because `src/exec.zig` already belongs to the `fucina` module. The build graph is itself one of the enforcement layers.
...
The tool parses every production source file, builds the local-import graph, runs Tarjan's strongly-connected-components algorithm over it, and fails on any forbidden SCC — any cycle other than the one permitted root-anchored shape (`P.zig` cycling with its own `P/` children, the struct-body-in-the-root pattern) — and on any band inversion. Current output:

```text
production import graph: 416 files, 1379 edges, 2 root-anchored SCC(s), 0 forbidden SCCs, 0 band inversions; 209 test files, all forwarded
```

Two subtleties make this checker honest rather than merely strict, and both are worth stealing for your own projects:

1. **It is test-aware.** Fucina's test convention (§16.5) has every production file pull in a sibling test file via `test { _ = @import("exec_tests.zig"); }` — and the sibling imports the production file back. That is a 2-cycle, and a naive cycle checker would flag every one of them. The tool instead excludes `@import`s inside `test` declarations and inside private test-only helpers, so the *production* graph it checks is the graph that actually ships. A lint that cries wolf gets deleted; a lint that understands the codebase's idioms survives.
...
Now the precision that matters, because acyclicity alone is easy to mistake for the whole contract: **zero SCCs does not imply correct band direction.** No production file participating in an import cycle says nothing about whether `exec` imports `ag` — a downward-pointing but band-violating edge is still perfectly acyclic. So arch-check checks direction separately, against the same table §16.1 shows, encoded as `band_table` in `tools/check_import_graph.zig`: every production file belongs to exactly one band, and every production import must land on a band at or below the importer's. A file in no band fails the check too, which is what keeps the table in the tool and the table in the docs from drifting apart. Both halves are machine gates; neither is a review invariant.
...
| Model forward passes | logit parity vs llama.cpp's `--save-logits` dumps, from *explicit raw token ids* so tokenizer bugs can't masquerade as model bugs | `apps/qwen3/main.zig` `--logits-out` / `--compare-logits`; `tools/llama_logits.cpp` |
...
**Runners double as oracle harnesses.** There is no separate "parity tool" directory that rots apart from the code it checks. Every model runner *is* the parity harness for its family: the qwen3 CLI exposes `--tokenize` (rung 1 of the ladder below), `--logits-out`/`--compare-logits` (rung 2), and `--verify-cache` alongside its ordinary `--chat`/`--gen`/`--bench` modes (`apps/qwen3/main.zig`); the multi-stage ports expose exit-code compare modes — parakeet's `--compare <stage> <dump>` flag, omnivoice's `compare` dump-diff subcommand (`apps/parakeet/main.zig`, `apps/omnivoice/main.zig`). The tool users run and the tool that proves correctness are the same binary, so the oracle cannot drift from the product.
...
Documentation lies in a specific, predictable way: it was true when written, then the API moved. Fucina closes that hole by making the reference manual executable. `zig build snippet-check` — an in-tree build step, run in CI on every push — extracts every runnable ```zig block from the `docs/reference/` chapters (any fenced block containing a column-0 `test "..."` declaration), generates a test root from them (`tools/gen_snippet_tests.zig`), and runs it against the real `fucina`/`fucina_models` modules with the build's option set (`docs/reference/02-toolchain-build-and-project-wiring.md` §2.7).

The authoring contract is small and worth copying:

- Snippets assume an **implicit prelude** (`std`, `fucina`, `models = @import("fucina_models")`, `optim = fucina.optim`) so each block stays short without becoming uncompilable.
...
- Snippets for **opt-in build features stay runnable, not skip-marked**: a constrained-decoding snippet opens with `if (!models.text.llguidance.enabled) return error.SkipZigTest;`, so it *compiles* under every flag combination and *executes* exactly when the feature is enabled.

This is why the course has told you, chapter after chapter, that reference snippets are machine-verified — it is not a figure of speech. When the public API changes, `snippet-check` fails until the documentation is updated, which makes updating the docs part of the change rather than a follow-up ticket that never happens ("Docs are part of the change", `docs/DEVELOPMENT.md` §4.4).

Two smaller pieces of documentation discipline orbit the same idea. `zig build doc-check` (`tools/check_doc_links.zig`) fails when the doc index in `docs/README.md` names a document that does not exist, when an intra-doc link or `src/<path>.zig:<line>` citation fails to resolve (and it existence-checks the `examples/<name>/README.md` and `apps/<name>/README.md` references in `docs/RUNNING-MODELS.md` the same way) — a five-minute tool that ends the era of dead links in the map. And the *style* contract for the docs themselves (`docs/DEVELOPMENT.md` §4.4): "Write docs as timeless reference — what exists and how it behaves; no dates (benchmark snapshots excepted), no development narrative." Reference documentation describes the present tree; history lives elsewhere. The one deliberate exception proves the rule twice over: benchmark records *must* carry dates (§16.6), and design records evolve by "dated outcome addenda rather than rewrites" (`docs/PORTING.md` §9) — so a reader can always tell what was believed when, and what superseded it, without the document lying about either.
...
**Sibling test files.** Behavior tests live in `<name>_tests.zig` next to the production file — 184 of them across `src/`, `examples/`, and `apps/`. The production file pulls its sibling in with a one-line forwarding stanza:
...
> **Zig note** — `error.SkipZigTest` is a language-level convention: a test that returns it is reported as *skipped*, distinct from both pass and fail. That distinction is what makes the discipline auditable — the run's summary shows exactly how many suites declined to run, so "green" never silently means "green because nothing executed". The idiomatic guard is one line at the top of the test: `if (!models.text.llguidance.enabled) return error.SkipZigTest;` for a feature flag, or a `catch |err| switch (err) { error.FileNotFound => return error.SkipZigTest, else => return err }` around an asset open.
...
6. `zig build snippet-check` — the reference runnable-snippet gate of §16.4;
...
| `zig build snippet-check` | every runnable reference snippet compiles and passes | any docs/reference edit; any public-API change |
...
Where each piece *lives* follows a placement policy (`docs/DEVELOPMENT.md` §1.8) that keeps growth from smearing the layers of §16.1: reusable engines other `src/models` consumers should import go in `src/models/<family>/`; single-purpose parity ports and their DSP/IO plumbing stay app-local in `apps/<name>/`; family-specific kernel orchestration lives in the family, never inside the generic exec runtime; shared cross-family scheduling lands once in exec and is re-exported. New core tensor ops belong in the exec/backend bands — "but a good port usually needs none: nanochat is entirely app-local over the public facade."
...
- `zig build arch-check` (in-tree, in CI) enforces **zero forbidden SCCs and zero band inversions** over the production import graph with an AST-based, test-aware checker — currently 416 files, 1379 edges; band *direction* is machine-checked against the layer table (`band_table`), and `docs/DEVELOPMENT.md` explicitly labels its remaining unenforced invariants review-only.
...
- `src/fucina.zig:95-110` — the comptime seal on the raw layer: a design rule as a compile error.
