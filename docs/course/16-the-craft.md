...
- `zig build arch-check` (in-tree, in CI) enforces **zero nontrivial SCCs** over the production import graph with an AST-based, test-aware checker — currently 135 files, 531 edges, 0 SCCs; band *direction* is review-enforced against the layer table, and `docs/DEVELOPMENT.md` explicitly labels its unenforced invariants review-only.
