...
- **Different regimes want different kernels.** The fused
  ...
  (`src/weights/linear.zig:327-336`) — a documented decision *with its
...
`MoeStreamOptions` (`src/weights/moe_stream.zig:9-15`, re-exported at
`runner.zig`) keeps experts on disk, read on
demand through a tiered cache — "the explicit trade that lets a
bigger-than-RAM model run at all" (`weights/moe_stream.zig:13-14`); that story (LRU
