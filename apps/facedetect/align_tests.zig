...
fn checkFace(allocator: std.mem.Allocator, comptime letter: []const u8) !u64 {
    ...
    const lbytes = try rec.readFile(std.testing.io, allocator, "apps/facedetect/goldens/align-lmk-" ++ letter ++ ".txt");
    ...
    const gbytes = try rec.readFile(std.testing.io, allocator, "apps/facedetect/goldens/crop-" ++ letter ++ ".bin");
