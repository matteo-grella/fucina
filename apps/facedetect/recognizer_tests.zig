...
fn checkFace(allocator: std.mem.Allocator, comptime letter: []const u8) !f64 {
    ...
    const gbytes = try recognizer.readFile(std.testing.io, allocator, "apps/facedetect/goldens/embed-" ++ letter ++ ".json");
    defer allocator.free(gbytes);
    const parsed = try std.json.parseFromSlice(Golden, allocator, gbytes, .{});
    defer parsed.deinit();

    const emb = try recognizer.embed(&ctx, allocator, std.testing.io, &file, "apps/facedetect/goldens/crop-" ++ letter ++ ".bin");
