...
```sh
zig build facedetect -Doptimize=ReleaseFast -- detect --model models/buffalo_l.gguf --input apps/facedetect/goldens/align-src-a.bin
zig build facedetect -Doptimize=ReleaseFast -- verify --model models/buffalo_l.gguf --a apps/facedetect/goldens/align-src-a.bin --b apps/facedetect/goldens/align-src-b.bin
```
...
- `bench --mode recognizer` expects a pre-aligned 112×112 crop as input
  (e.g. `apps/facedetect/goldens/crop-a.bin`), mirroring the reference's
