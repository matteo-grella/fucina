...
test "cli: detect JSON matches reference (face_a)" {
    ...
    var img = try cli.readImage(allocator, std.testing.io, "apps/facedetect/goldens/align-src-a.bin");
    defer img.deinit();
    ...
    defer mine.deinit();
    const gbytes = try rec.readFile(std.testing.io, allocator, "apps/facedetect/goldens/detect-a.txt");
