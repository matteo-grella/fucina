...
fn checkFace(allocator: std.mem.Allocator, comptime letter: []const u8) !f32 {
    ...
    var heads = try scrfd.forward(&ctx, allocator, std.testing.io, &file, "apps/facedetect/goldens/det-blob-" ++ letter ++ ".bin");
    defer heads.deinit(allocator);

    const sbytes = try rec.readFile(std.testing.io, allocator, "apps/facedetect/goldens/det-scale-" ++ letter ++ ".txt");
    ...
    const gbytes = try rec.readFile(std.testing.io, allocator, "apps/facedetect/goldens/detect-" ++ letter ++ ".txt");
