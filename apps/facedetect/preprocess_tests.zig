...
fn checkFace(allocator: std.mem.Allocator, comptime letter: []const u8) !u64 {
    const sbytes = rec.readFile(std.testing.io, allocator, "apps/facedetect/goldens/align-src-" ++ letter ++ ".bin") catch |err| switch (err) {
    ...
    const dsbytes = try rec.readFile(std.testing.io, allocator, "apps/facedetect/goldens/det-scale-" ++ letter ++ ".txt");
    ...
    const gbytes = try rec.readFile(std.testing.io, allocator, "apps/facedetect/goldens/det-blob-" ++ letter ++ ".bin");
