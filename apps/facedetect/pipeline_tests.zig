...
fn embedFace(ctx: *ExecContext, al: std.mem.Allocator, file: *const gguf.File, comptime letter: []const u8) ![]f32 {
    const sbytes = try rec.readFile(std.testing.io, al, "apps/facedetect/goldens/align-src-" ++ letter ++ ".bin");
...
test "embed: end-to-end production cosine >= 0.9999 vs reference (face_a/b/c)" {
    ...
    inline for (.{ "a", "b", "c" }) |letter| {
        ...
        const gbytes = try rec.readFile(std.testing.io, allocator, "apps/facedetect/goldens/embed-" ++ letter ++ ".json");
...
test "analyze: end-to-end gender exact + age vs reference (face_a/b/c)" {
    ...
    inline for (acases) |cs| {
        ...
        const sbytes = try rec.readFile(std.testing.io, allocator, "apps/facedetect/goldens/align-src-" ++ cs.letter ++ ".bin");
