...
fn checkCase(allocator: std.mem.Allocator, comptime cs: Case) !f32 {
    ...
    const pts = try landmarks.landmarks(&ctx, allocator, std.testing.io, &file, "apps/facedetect/goldens/lm-crop-" ++ cs.letter ++ "-" ++ cs.tag ++ ".bin", cs.three_d);
    defer allocator.free(pts);

    const gbytes = try rec.readFile(std.testing.io, allocator, "apps/facedetect/goldens/lm-pts-" ++ cs.letter ++ "-" ++ cs.tag ++ ".txt");
