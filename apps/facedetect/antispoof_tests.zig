...
test "antispoof: MiniFASNet replay real_prob vs reference (face_a/b/c)" {
    ...
    inline for (cases) |cs| {
        ...
        const crops = [_][]const u8{
            "apps/facedetect/goldens/as-crop-" ++ cs.letter ++ "-0.bin",
            "apps/facedetect/goldens/as-crop-" ++ cs.letter ++ "-1.bin",
        };
...
test "antispoof: crop-from-bbox matches the reference dumps; argmax-gated score (face_a)" {
    ...
    var img = try loadFixture(allocator, "apps/facedetect/goldens/align-src-a.bin");
    defer img.deinit();
    ...
    const crop_goldens = [_][]const u8{
        "apps/facedetect/goldens/as-crop-a-0.bin",
        "apps/facedetect/goldens/as-crop-a-1.bin",
    };
...
test "antispoof: verify a-b liveness veto keeps the reference verdict" {
    ...
    var ia = try loadFixture(allocator, "apps/facedetect/goldens/align-src-a.bin");
    defer ia.deinit();
    var ib = try loadFixture(allocator, "apps/facedetect/goldens/align-src-b.bin");
    defer ib.deinit();
    ...
    const golden = try rec.readFile(std.testing.io, allocator, "apps/facedetect/goldens/verify-ab-antispoof.txt");
