...
//! shared by every audio-facing app (the `audio_io` module in build.zig).
...
extern fn fucina_audio_create() ?*Handle;
extern fn fucina_audio_destroy(audio: ?*Handle) void;
extern fn fucina_audio_list_devices(audio: ?*Handle, kind: c_int, name_buf: [*]u8, name_cap_arg: c_int, default_flags: [*]u8, cap: c_int) c_int;
extern fn fucina_audio_start(audio: ?*Handle, capture_index: c_int, playback_index: c_int, sample_rate: c_uint, period_frames: c_uint, callback: RawCallback, user: ?*anyopaque) c_int;
extern fn fucina_audio_start_capture(audio: ?*Handle, capture_index: c_int, sample_rate: c_uint, period_frames: c_uint, callback: RawCallback, user: ?*anyopaque) c_int;
extern fn fucina_audio_stop(audio: ?*Handle) void;
extern fn fucina_audio_actual_sample_rate(audio: ?*Handle) c_uint;
extern fn fucina_audio_internal_sample_rate(audio: ?*Handle, kind: c_int) c_uint;
extern fn fucina_audio_device_latency_frames(audio: ?*Handle, kind: c_int) c_uint;
extern fn fucina_audio_internal_period_frames(audio: ?*Handle, kind: c_int) c_uint;
extern fn fucina_audio_running_names(audio: ?*Handle, capture_name: [*]u8, playback_name: [*]u8, cap: c_int) void;
...
pub const Audio = struct {
    ...
    pub fn init() !Audio {
        const handle = fucina_audio_create() orelse return error.AudioContextInit;
        return .{ .handle = handle };
    }

    pub fn deinit(self: *Audio) void {
        fucina_audio_destroy(self.handle);
        self.* = undefined;
    }
    ...
    pub fn listDevices(self: *Audio, kind: DeviceKind, out: *[max_devices]DeviceInfo) ![]DeviceInfo {
        ...
        const count = fucina_audio_list_devices(self.handle, @intFromEnum(kind), @ptrCast(&names), name_cap, &defaults, max_devices);
    ...
    pub fn startCapture(self: *Audio, capture: usize, sample_rate: u32, period_frames: u32, callback: RawCallback, user: ?*anyopaque) !void {
        const rc = fucina_audio_start_capture(self.handle, @intCast(capture), sample_rate, period_frames, callback, user);
        if (rc != 0) return error.CaptureStart;
    }

    pub fn stop(self: *Audio) void {
        fucina_audio_stop(self.handle);
    }

    pub fn actualSampleRate(self: *Audio) u32 {
        return fucina_audio_actual_sample_rate(self.handle);
    }
    ...
    pub fn internalSampleRate(self: *Audio, kind: DeviceKind) u32 {
        return fucina_audio_internal_sample_rate(self.handle, @intFromEnum(kind));
    }
    ...
    pub fn deviceLatencyFrames(self: *Audio, kind: DeviceKind) u32 {
        return fucina_audio_device_latency_frames(self.handle, @intFromEnum(kind));
    }

    /// The device-side period miniaudio actually negotiated.
    pub fn internalPeriodFrames(self: *Audio, kind: DeviceKind) u32 {
        return fucina_audio_internal_period_frames(self.handle, @intFromEnum(kind));
    }

    pub fn runningNames(self: *Audio, capture_buf: *[name_cap]u8, playback_buf: *[name_cap]u8) void {
        fucina_audio_running_names(self.handle, capture_buf, playback_buf, name_cap);
    }
};
