...
const CqT = Tensor(.{ .qo, .d });
const CkvT = Tensor(.{ .kvo, .d });
const CProjAttnT = Tensor(.{ .d, .attn });
const CfcT = Tensor(.{ .ff, .d });
const CProjMlpT = Tensor(.{ .d, .ff });
const VeGateT = Tensor(.{ .kv_head, .d }); // [n_kv_head, 12]
...
pub const Model = struct {
    ...
    pub fn initFromSafetensors(cfg: Config, ctx: *ExecContext, allocator: Allocator, io: std.Io, path: []const u8) !Model {
        ...
        for (0..cfg.n_layer) |i| {
            const l = &layers[i];
            l.ve_gate = null;
            l.c_q = try loadParam(.{ .qo, .d }, ctx, allocator, &file, try lname(&name_buf, i, "attn.c_q.weight"), [_]usize{ qo, d });
            errdefer l.c_q.deinit();
            l.c_k = try loadParam(.{ .kvo, .d }, ctx, allocator, &file, try lname(&name_buf, i, "attn.c_k.weight"), [_]usize{ kvo, d });
            errdefer l.c_k.deinit();
            l.c_v = try loadParam(.{ .kvo, .d }, ctx, allocator, &file, try lname(&name_buf, i, "attn.c_v.weight"), [_]usize{ kvo, d });
            errdefer l.c_v.deinit();
            l.c_proj = try loadParam(.{ .d, .attn }, ctx, allocator, &file, try lname(&name_buf, i, "attn.c_proj.weight"), [_]usize{ d, qo });
            errdefer l.c_proj.deinit();
            l.c_fc = try loadParam(.{ .ff, .d }, ctx, allocator, &file, try lname(&name_buf, i, "mlp.c_fc.weight"), [_]usize{ ff, d });
            errdefer l.c_fc.deinit();
            l.c_proj_mlp = try loadParam(.{ .d, .ff }, ctx, allocator, &file, try lname(&name_buf, i, "mlp.c_proj.weight"), [_]usize{ d, ff });
            errdefer l.c_proj_mlp.deinit();
            if (cfg.hasVe(i)) {
                l.ve_gate = try loadParam(.{ .kv_head, .d }, ctx, allocator, &file, try lname(&name_buf, i, "attn.ve_gate.weight"), [_]usize{ cfg.n_kv_head, 12 });
    ...
    pub fn initRandom(cfg: Config, ctx: *ExecContext, allocator: Allocator, seed: u64) !Model {
        const d = cfg.n_embd;
        const hd = cfg.headDim();
        const qo = cfg.n_head * hd;
        const kvo = cfg.n_kv_head * hd;
        ...
        for (0..cfg.n_layer) |i| {
            const l = &layers[i];
            l.ve_gate = null;
            l.c_q = try rngParam(.{ .qo, .d }, ctx, allocator, [_]usize{ qo, d }, .{ .uniform = .{ .lo = -s, .hi = s } }, fucina.rng.at(seed, k));
            errdefer l.c_q.deinit();
            k += 1;
            l.c_k = try rngParam(.{ .kvo, .d }, ctx, allocator, [_]usize{ kvo, d }, .{ .uniform = .{ .lo = -s, .hi = s } }, fucina.rng.at(seed, k));
            errdefer l.c_k.deinit();
            k += 1;
            l.c_v = try rngParam(.{ .kvo, .d }, ctx, allocator, [_]usize{ kvo, d }, .{ .uniform = .{ .lo = -s, .hi = s } }, fucina.rng.at(seed, k));
            errdefer l.c_v.deinit();
            k += 1;
            l.c_proj = try rngParam(.{ .d, .attn }, ctx, allocator, [_]usize{ d, qo }, .zeros, 0);
            errdefer l.c_proj.deinit();
            l.c_fc = try rngParam(.{ .ff, .d }, ctx, allocator, [_]usize{ ff, d }, .{ .uniform = .{ .lo = -s * 0.4, .hi = s * 0.4 } }, fucina.rng.at(seed, k));
            errdefer l.c_fc.deinit();
            k += 1;
            l.c_proj_mlp = try rngParam(.{ .d, .ff }, ctx, allocator, [_]usize{ d, ff }, .zeros, 0);
            errdefer l.c_proj_mlp.deinit();
            if (cfg.hasVe(i)) {
                l.ve_gate = try rngParam(.{ .kv_head, .d }, ctx, allocator, [_]usize{ cfg.n_kv_head, 12 }, .{ .uniform = .{ .lo = 0, .hi = 0.02 } }, fucina.rng.at(seed, k));
                errdefer if (l.ve_gate) |*g| g.deinit();
                k += 1;
    ...
    pub fn forward(self: *const Model, ctx: *ExecContext, token_ids: []const usize, trace: ?*Trace) !Tensor(.{ .seq, .vocab }) {
    ...
    fn forwardTrunk(self: *const Model, ctx: *ExecContext, token_ids: []const usize, trace: ?*Trace) !Tensor(.{ .seq, .d }) {
        const cfg = self.cfg;
    ...
    pub fn forwardStep(self: *const Model, ctx: *ExecContext, cache: *Cache, token_ids: []const usize, pos0: usize) !Tensor(.{ .seq, .vocab }) {
        const cfg = self.cfg;
    ...
    pub fn loss(self: *const Model, ctx: *ExecContext, token_ids: []const usize, targets: []const isize) !Tensor(.{}) {
    ...
    pub fn lossNone(self: *const Model, ctx: *ExecContext, token_ids: []const usize, targets: []const isize) !Tensor(.{.seq}) {
        const labels = try self.allocator.alloc(usize, targets.len);
    ...
    pub fn lossSum(self: *const Model, ctx: *ExecContext, token_ids: []const usize, labels: []const usize) !Tensor(.{}) {
        const logits = try self.forward(ctx, token_ids, null);
...
fn loadParam(
    ...
    return Tensor(tags).variableFromSlice(ctx, shape, tmp);
}
...
fn rngParam(
    ...
    return Tensor(tags).variableFromSlice(ctx, shape, buf);
}
